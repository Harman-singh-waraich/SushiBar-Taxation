...
contract SushiBar is ERC20("SushiBar", "xSUSHI"){
    ...
    function leave(uint256 _share) public {
        require(block.timestamp.sub(StakeTime[msg.sender]) >= 2*86400, "Cannot unstake before two days");
        // Gets the amount of xSushi in existence
        uint256 totalShares = totalSupply();
    ...
    function allowed(address user,uint256 _share) private view returns(bool){
        ...
        if(2*DAY <= timePassed && timePassed <4*DAY){
            ...
            require(usersTotalShares.mul(25).div(100) >=_share , "Cannot unstake more than 25% before 4 days");
            return true ;                 

        }else if(4*DAY <= timePassed && timePassed <6*DAY){
            
            require(usersTotalShares.mul(50).div(100) >=  _share , "Cannot unstake more than 50% before 6 days");
            return true ;                                  
        
        }else if(6*DAY<= timePassed && timePassed <8*DAY){
        
            require(usersTotalShares.mul(75).div(100) >= _share , "Cannot unstake more than 75% before 8 days");
            return true ;                                  
